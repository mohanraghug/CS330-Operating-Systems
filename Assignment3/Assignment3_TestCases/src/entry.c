...
int sys_count_breakpoints(struct exec_context *ctx, u64 addr)
{
	int count = 0;
	struct breakpoint_info *itr = ctx->dbg->head;
	while (itr != NULL) {
		if (itr->addr == addr) count++;
		itr = itr->next;
	}
	return count;
}

int sys_check_enabled_breakpoint(struct exec_context *ctx, u64 addr)
{
	int enabled = 0;
	//printk("Checking for addr = %x\n", addr);
	struct breakpoint_info *itr = ctx->dbg->head;
	while (itr != NULL) {
		// printk("addr found = %x\n", itr->addr);
		if (itr->addr == addr) {
	...
	/*if(enabled)
		printk("Enabled and found\n");
	else
		printk("Not (Enabled and found)\n");
	*/
	return enabled;
}
...
long  do_syscall(int syscall, u64 param1, u64 param2, u64 param3, u64 param4)
{
	...
	case SYSCALL_COUNT_BREAKPOINTS:
		return sys_count_breakpoints(current, param1);
	case SYSCALL_CHECK_ENABLED_BREAKPOINTS:
		return sys_check_enabled_breakpoint(current, param1);
	default:
		return -1;
	}
