...
int main(u64 arg1, u64 arg2, u64 arg3, u64 arg4, u64 arg5)
{
  ...
  // Deallocating all the vm_area with READs (MIddle one) in reverse order
  for(int i =(total-2); i >= 0; i= i - 3)
  {
      char* vm_area_start = mm1 + (i * (pages*100)) + pages*10;
      util_munmap(vm_area_start, pages*80);
  }
  // Vm_area count should be  84;
  pmap(0);



  // Deallocating all the vm_area with WRITE PROT
  for(int i =0; i < total; i = i + 3)
  {
      char* vm_area_start = mm1 + (i * (pages*100)) + pages*10;
      util_munmap(vm_area_start, pages*80);
  }
  // Vm_area count should be  42;
  pmap(0);
