...
int call_get_fileobj_count(struct exec_context *ctx)
{
	return stats->file_objects;
}

int call_is_msg_queue(struct exec_context *ctx, int fd)
{
...
long  do_syscall(int syscall, u64 param1, u64 param2, u64 param3, u64 param4)
{
	...
	// for testing
	case SYSCALL_GET_FILEOBJ_COUNT:
		return call_get_fileobj_count(current);
	case SYSCALL_IS_MSG_QUEUE:
		return call_is_msg_queue(current, param1);
